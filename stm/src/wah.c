...
int wah_init(t_wah_data *data, int blocksize, int sampling_rate, float freq_low, float freq_high, float freq_delta, float Q) {
    ...
    data->freq_delta = freq_delta; // wah frequency, how many Hz are cycled through pr sample
    data->freq_center = data->freq_low; // pass band center freq
    data->Q = Q; // pass band width, lower value means smaller band
...
void wah_apply(t_wah_data *data, float *input) {    
	...
	if(data->enable == WAH_PASSTHROUGH)
	{
		arm_copy_f32(input, data->output, data->blocksize);
	}
	else if(data->enable == WAH_ENABLE)
	{
		float F1, yh, yb, yl;
